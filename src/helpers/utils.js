...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in TherapistTube channel by the TherapistTube Admin.
  To access your channel, log into your TherapistTube account,click on the
  TherapistTube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `TherapistTube has approved your Therapist application.`;
    } else {
      msg = `TherapistTube has unapproved your Therapist application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
...
exports.sendAppointmentMailToUser = async (data) => {
  ...
  const query1 =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values1 = [data.therapistProfileId];
